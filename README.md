...
## Version 1.6.1 — Collaboration + Prime Stability Update
...
- Invite links prefill the room code and open the APS side panel when v1.6.1 is installed
...
1. Click the link with APS v1.6.1 installed.
...
The invitation page also displays the normal room code, so joining by code remains available. The link cannot silently install the unpacked Chrome extension; the friend must install v1.6.1 first.
...
This update changes both the extension and the server. Upload the complete v1.6.1 source to the GitHub repository. Render's `autoDeploy: true` setting should redeploy the server automatically. Wait for **Live** before sharing the new package.
...
Version 1.6.1 is Chrome-first on macOS, Windows and desktop ChromeOS. Mobile Chrome cannot install this unpacked extension. Safari would require separate Xcode packaging and live platform testing.
